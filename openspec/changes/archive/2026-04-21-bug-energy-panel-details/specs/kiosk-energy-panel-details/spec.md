...
For each appliance in the clicked cell's `d.appliances`, the panel SHALL compute `powerRaw` as the sum of all numeric values delivered by the current detail `transport-update` for that `applianceId` (treating non-numeric or NaN values as 0). It SHALL render each entry as the appliance's resolved name — falling back to its `id` as a string when no name has been resolved via the appliances service — together with `powerRaw` formatted through the project's power-formatting utility.
...
#### Scenario: Appliance with no delivered values

- **WHEN** an appliance is in the clicked cell's `d.appliances` but the `transport-update` carries no value for it
...
When the user taps the back face, the panel SHALL flip the card back to its front face, deregister the active detail SSE transport via `sse-transport-client`, and empty the rendered detail list. After close, no callback from the closed transport SHALL modify the rendered detail list.
...
When `KioskPowerPanel` is destroyed, any active or in-flight detail SSE transport SHALL be deregistered via `sse-transport-client`, and no callback from such a transport SHALL modify component state after destruction.
